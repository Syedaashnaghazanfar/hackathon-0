#!/usr/bin/env python3
"""
Twitter/X Login Helper - Authenticate and save browser session for social media automation

This script opens a browser to Twitter/X login page and waits for manual authentication.
Once authenticated, the session is saved to .social_session/twitter/ for reuse.
...
async def login_twitter():
    """
    Open Twitter/X login page and wait for user to authenticate.

    Session is automatically saved to .social_session/twitter/ for reuse.
    """
    platform_dir = SESSION_DIR / PLATFORM
    platform_dir.mkdir(parents=True, exist_ok=True)

    logger.info(f"Starting Twitter/X login helper")
    ...
    try:
        ...
        logger.info(f"Navigating to {TWITTER_URL}")
        await page.goto(TWITTER_URL, timeout=30000)

        # Wait for user to log in
        logger.info("")
        logger.info("=" * 70)
        logger.info("BROWSER OPENED - Please log in to Twitter/X")
        logger.info("=" * 70)
        logger.info("")
        logger.info("Instructions:")
        logger.info("1. Log in to Twitter/X with your credentials")
        ...
        try:
            ...
            logger.info("")
            logger.info("Session saved. You can now use Twitter posting tools.")
            logger.info("")
        except:
        ...
        logger.info("- Use MCP tool: post_to_twitter")
